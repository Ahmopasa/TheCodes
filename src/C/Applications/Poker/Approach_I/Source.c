...
int main()
{
	...
	if (*user == NULL)
	{
		puts("1-) Memor Allocation was failed.");
		exit(1);
	}
	for (int i = 0; i < 8; i++)
	{
		...
		if (*user == NULL)
		{
			puts("2-) Memor Allocation was failed.");
			exit(1);
		}
	}
		...
		switch (turn_counter)
		{
			case 1: 
			{
				...
				for (int i = PlayerToPlay; i < ((PlayerToPlay + total_user)); i++)
				{
					...
					try_again: printf("It us player %d's turn.\nChoose one below;\n(1) -Fold your cards.\n(2) -Raise the blind.\n(3) -Call.\nChoice: ", the_player);
				...
				if (flag)
				{
					...
					for (int i = the_player; (i) < (total_user + the_player); i++)
					{
						...
						{
							try_again_2: printf("Player %d, choose below. You need to make a new blind. Min. %d.\n", i % total_user, big_blind);
							printf("(1) - To Fold\n");
							printf("(2) - To Raise\n");
							printf("(3) - To Call\n");
							if (scanf("%d", &player_action));
